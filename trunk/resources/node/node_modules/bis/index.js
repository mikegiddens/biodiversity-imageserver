...
var BIS = function(params){
	...
	console.log('In Load By Id Loop');
	...
	function imageSave(params,callback) {
		...
		imageExistsById(params.image_id, function(data) {
	...
	function createThumb(params,callback) {
		...
		if(params.imagePath == '' || params.width == '' || params.height == '') {
			...
			console.log('Thumb Creation ');
				// to preserve aspect ratio
				gm( params.imagePath ).size(function(err, value){
				// console.log('Image size');
				// console.log(value);
					var width,height;
					var destination = path.normalize(path.dirname(params.imagePath) + '\\' + path.basename(params.imagePath,path.extname(params.imagePath)) + params.postFix + path.extname(params.imagePath));

					// console.log('Source : ' + params.imagePath);
	...
	function deleteProcessQueue(params,callback) {
		params = ('undefined' != typeof params) ? params : '';
	...
	function processType(record,callback) {
		...
		loadImageById(record.image_id,function(imageRow){
			if(false == imageRow) {
				return callback(false);
			} else {
				var imagePath = path.normalize(storageDevices[imageRow.storage_id].basePath + imageRow.path + '\\' + imageRow.filename);
				switch(record.process_type) {
					case 'all':
						if(storageDevices[imageRow.storage_id].type == 's3') {} else {
							console.log('To thumb Creation');
		...
		var processFn = function() {
			if(loopFlag) {
				me.popQueue({},function(record){
					if(false != record) {
					console.log('Pop Queue Successful');
						me.processType(record,function(status){
