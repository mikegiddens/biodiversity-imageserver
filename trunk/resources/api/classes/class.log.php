...
class LogClass
{
	...
	*/
	...
	public function load_by_id( $logId ) {
		if($geoId == '') return false;
		$query = sprintf("SELECT * FROM `log` WHERE `id` = %s ", mysql_escape_string($logId) );
	...
	public function listRecords($queryFlag = true) {
		...
		if($this->data['logId'] != '') {
			$where .= sprintf(" AND `id` = '%s' ", mysql_escape_string($this->data['logId']));
		}
		if($this->data['action'] != '') {
		...
		$query = "SELECT SQL_CALC_FOUND_ROWS `id`,`action`,`table`,`query`,`lastModifiedBy`,`modifiedTime` FROM  `log` " . $where;
	...
	public function recordExists ($logId){
		if($logId == '' || is_null($logId)) return false;
		$query = sprintf("SELECT `id` FROM `log` WHERE `id` = %s;", mysql_escape_string($logId) );
		$ret = $this->db->query_one( $query );
		if ($ret == NULL) {
			return false;
		} else {
	...
	public function save() {
		...
		, mysql_escape_string($this->get('action'))
		, mysql_escape_string($this->get('table'))
		, mysql_escape_string($this->get('query'))
