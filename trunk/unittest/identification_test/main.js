		...
		if(data.length > 128){
			...
			if(base64StartIndex < data.length) {
				...
				$.ajax({
					...
					data: postString, // Just send the Base64 content in POST body
					...
					success: function onUploadComplete(response) {
						...
						if(response.success) {
							...
							$.ajax({
								url: '../../resources/api/api.php?cmd=addImageToCollection&imageId='+response.image_id+'&collectionCode=ECN',
								success: function(data, textStatus, jqXHR) {
									console.log('Collection Added');
									console.log(data);
									$.ajax({
										url: '../../resources/api/api.php?cmd=add_image_attribute&imageID='+response.image_id+'&categoryID=59&valueID=62',
										success: function(data, textStatus, jqXHR) {
							...
							markup += "Image Id : "+response.image_id;
							markup += "  </div>";
