...
var SilverTiles = function(params){
	...
	var baseUrl = 'http://localhost/bis/resources/node/cacheFolder/';
...
/*		
		...
		for(var j=0; j<x; j++) {
			for(var i=0; i<x; i++) {
				// console.log(i*tSize, j*tSize);
				// console.log(' k : ' + k);
				
				gm(source).resize(width,height).extent(width,height).crop(tSize,tSize,i*tSize,j*tSize).write(tile2 + 'tile_' + k + '.jpg', function (err) {console.log(err);});
...
*/
		...
		var tileFn1 = function() {
			if(k < Math.pow(zoom,4) - 1) {
				k++;
				if(l<Math.pow(zoom,2) - 1) {
					l++;
				} else {
					l = 0; m++;
				}
				// console.log(i, j, k, i*tSize, j*tSize);
				gm(source).resize(width,height).extent(width,height).crop(tSize,tSize,l*tSize,m*tSize).write(tile2 + 'tile_' + k + '.jpg', function (err) {console.log(err);});
				tileFn1();
			}
		}
		// tileFn1();
	...
	function touchCache(callback) {
		var me = this;
		this.getTileLevels(function(zoom){
			// fs.utimesSync(me.getTileLocation(),Date.now(),Date.now());
	...
	function setData(params) {
		params = ('undefined' != typeof params) ? params : '';
		tileSize = ('undefined' != typeof params.tileSize) ? params.tileSize : tileSize;
		sourcePath = ('undefined' != typeof params.sourcePath) ? params.sourcePath : sourcePath;
		image = ('undefined' != typeof params.image) ? params.image : image;
	}
	...
	function createTileLocation() {
		mkdirp.sync(this.getTileLocation());
		return true;
	}
					...
					if(false != zoom) {
						...
						var tileFn = function() {
							...
							if(j>=1){
								...
								if(j == 1) {
									...
									var tileFn1 = function() {
										if(k < Math.pow(j,4) - 1) {
											...
											// console.log(i, j, k, i*tSize, j*tSize);
											gm(source).resize(width,height).extent(width,height).crop(tileSize,tileSize,l*tileSize,m*tileSize).write(tmpCachePath + 'tile_' + k + '.jpg', function (err) {
												if(!err) {
													tileFn1();
												} else {
													console.log(err);
								...
								return callback({success : true, processTime : '', copyright : 'COPYRIGHT', url : baseUrl + path.basename(image,'.jpg').toLowerCase() + '/', width : originalDimensions.width, height : originalDimensions.height, zoomLevel : zoom});
							}
						}
						tileFn();
					} else {
